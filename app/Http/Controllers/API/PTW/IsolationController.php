...
class IsolationController extends BaseController
{
    /**
     * UAUC api
     *
    ...
    public function list(Request $request)
    {
        ...
        if (Auth::user()) {
            ...
            $ptw_list_array = $this->isolation->select('ptw_sub_permit_isolation.*', 'master_location.location_name', 'users.name');
            $ptw_list_array = $ptw_list_array->leftJoin('master_location', 'ptw_sub_permit_isolation.location', '=', 'master_location.id');
            $ptw_list_array = $ptw_list_array->leftJoin('master_location_specific', 'ptw_sub_permit_isolation.location', '=', 'master_location_specific.id');
            $ptw_list_array = $ptw_list_array->leftJoin('users', 'ptw_sub_permit_isolation.created_by', '=', 'users.id');
            ...
            if (CheckUserRole(ROLE_CONTRACTORADMIN)) {
                ...
                $ptw_list_array = $ptw_list_array->where('ptw_sub_permit_isolation.created_by', Auth::id());
            }
            ...
            foreach ($ptw_list_array as $listdata) {
                ...
                $data['status_id'] = $listdata->status;
            ...
            return $this->sendResponse($success, 'Isolation PTW Details');
        } else {
    ...
    public function view(Request $request)
    {
        ...
        try {
            ...
            if (Auth::user()) {
                ...
                $isolation = $this->isolation->selectOne($id);

                $isolationDetails =    $this->general->isolationView($isolation->ptw_id);
