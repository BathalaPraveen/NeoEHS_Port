...
class LiftingController extends BaseController
{
    /**
     * UAUC api
     *
    ...
    public function list(Request $request)
    {
        ...
        if (Auth::user()) {
            ...
            $ptw_list_array = $this->lifting->select('ptw_sub_permit_lifting.*', 'master_location.location_name', 'users.name');
            $ptw_list_array = $ptw_list_array->leftJoin('master_location', 'ptw_sub_permit_lifting.location', '=', 'master_location.id');
            $ptw_list_array = $ptw_list_array->leftJoin('users', 'ptw_sub_permit_lifting.created_by', '=', 'users.id');
            ...
            if (CheckUserRole(ROLE_CONTRACTORADMIN)) {
                ...
                $ptw_list_array = $ptw_list_array->where('ptw_sub_permit_lifting.created_by', Auth::id());
            }
            ...
            return $this->sendResponse($success, 'Lifting PTW Details');
        } else {
    ...
    public function view(Request $request)
    {
        ...
        try {
            ...
            if (Auth::user()) {
                ...
                $lifting = $this->lifting->selectOne($id);

                $liftingDetails =    $this->general->liftingView($lifting->ptw_id);
