...
class GasController extends BaseController
{
    ...
    public function list(Request $request)
    {
        ...
        if (Auth::user()) {
            ...
            $ptw_list_array = $this->gas->select('ptw_sub_permit_gas.*', 'master_location.location_name', 'users.name');
            $ptw_list_array = $ptw_list_array->leftJoin('master_location', 'ptw_sub_permit_gas.location', '=', 'master_location.id');
            $ptw_list_array = $ptw_list_array->leftJoin('master_location_specific', 'ptw_sub_permit_gas.location', '=', 'master_location_specific.id');
            $ptw_list_array = $ptw_list_array->leftJoin('users', 'ptw_sub_permit_gas.created_by', '=', 'users.id');
            ...
            if (CheckUserRole(ROLE_CONTRACTORADMIN)) {
                ...
                $ptw_list_array = $ptw_list_array->where('ptw_sub_permit_gas.created_by', Auth::id());
            }
            ...
            return $this->sendResponse($success, 'Gas PTW Details');
        } else {
    ...
    public function view(Request $request)
    {
        ...
        try {
            ...
            if (Auth::user()) {
                ...
                $gas = $this->gas->selectOne($id);

                $gastestDetails =    $this->general->gasView($gas->ptw_id);
                ...
                return $this->sendResponse($success, 'Gas Test PTW Details');
            } else {
                return $this->sendError('Unauthorised.', ['error' => 'Unauthorised'], 401);
            }
        } catch (Exception $ex) {
