...
class HotworkController extends BaseController
{
    /**
     * UAUC api
     *
    ...
    public function list(Request $request)
    {
        ...
        if (Auth::user()) {
            ...
            $ptw_list_array = $this->hotwork->select('ptw_sub_permit_hotwork.*', 'master_location.location_name', 'users.name');
            $ptw_list_array = $ptw_list_array->leftJoin('master_location', 'ptw_sub_permit_hotwork.location', '=', 'master_location.id');
            $ptw_list_array = $ptw_list_array->leftJoin('users', 'ptw_sub_permit_hotwork.created_by', '=', 'users.id');
            ...
            if (CheckUserRole(ROLE_CONTRACTORADMIN)) {
                ...
                $ptw_list_array = $ptw_list_array->where('ptw_sub_permit_hotwork.created_by', Auth::id());
            }
            ...
            return $this->sendResponse($success, 'Hotwork PTW Details');
        } else {
    ...
    public function view(Request $request)
    {
        try {
            ...
            if (Auth::user()) {
                ...
                $hotwork = $this->hotwork->selectOne($id);

                $hotworkDetails =    $this->general->hotworkView($hotwork->ptw_id);
