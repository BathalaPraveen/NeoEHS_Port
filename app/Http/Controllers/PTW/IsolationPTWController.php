...
use App\Mail\PTW\SWPIsolationEmail;

class IsolationPTWController extends Controller
{
    ...
    public function index(Request $request)
    {
                ...
                try {
                    ...
                    $datatables = Datatables::of($data['data'])
                        ...
                        ->addColumn('action', function ($row) {
                            $btn = '';
                            $btn = '<a href="' . admin_url('ptw/isolation/view/' . encryptId($row->id)) . '"   class="" title="View"><i class="fa-solid fa-eye"></i></a> ';
                            if ($row->ptw_status == SUBPERMIT_STATUS_PENDING  && (CheckUserRole(ROLE_ISOLATION_PERMIT_APPROVER) || isAdmin() || CheckUserRole(ROLE_HSEUSER))) {
                                $btn .= '<a href="' . admin_url('ptw/isolation/approvereject/' . encryptId($row->id)) . '"   class="" title="Approve / Reject"><i class="fa-solid fa-check-to-slot"></i></a> ';
                            }
                            if ($row->ptw_status == SUBPERMIT_STATUS_REJECTED && (Auth::id() == $row->created_by || isAdmin())) {
                                $btn .= '<a href="' . admin_url('ptw/isolation/edit/' . encryptId($row->id)) . '"   class="" title="Edit"><i class="fa-solid fa-pen-to-square"></i></a> ';
                            }
                            $btn .= '<a href="' . admin_url('ptw/isolation/view/pdf/' . encryptId($row->id)) . '" class=" " title="Pdf"><img style="width:20px;vertical-align: sub;" src="' . url('public/assets/images/icons/pdf.png') . '" alt="PDF" ></i></a> ';
                            return $btn;
                        })
        ...
        return view('ptw.isolation.list', $data);
    }
    ...
    public function Add(Request $request)
    {
        ...
        try {
            ...
            $nextpermit = $this->ptwsubpermit->nextpermit($ptwid, PTW_SUB_PERMIT_ISOLATION);
            ...
            return view('ptw.isolation.add', $data);
        } catch (Exception $ex) {
            report($ex);
        }
    }
    ...
    public function Store(Request $request)
    {
        try {
            ...
            $rules = [
                // 'durationofisolation' => 'required',
                'workdescription' => 'required',
                'whatdoisolate' => 'required',
                'sourceofenergy' => 'required',
                'loockoutapplied' => 'required',
                'icno' => 'required',
            ...
            $messages = [
                //  'durationofisolation.required' => 'Please enter ',
                'workdescription.required' => 'Please enter ',
                'whatdoisolate.required' => 'Please enter ',
                'sourceofenergy.required' => 'Please enter ',
                'loockoutapplied.required' => 'Please enter ',
                'icno.required' => 'Please enter ',
                'accept_terms.required' => 'Please enter ',
                'applicant_remarks.required' => 'Please enter ',
            ...
            try {
                ...
                $isolation = $this->isolation->store($general);

                $this->ptwsubpermit->updatePermit($ptwid, PTW_SUB_PERMIT_ISOLATION);
                ...
                $user_role = ROLE_ISOLATION_PERMIT_APPROVER;
                ...
                if (count($users) > 0) {
                    ...
                    foreach ($users as $user) {
                        ...
                        $mailsubject = "[PTW Notification - " . PTWID($ptw_id) . " ] " .  'New Isolation PTW Request';


                        if ($email_id != '' || $email_id != null) {

                            $isolationdetails =  $this->isolation->selectOne($isolation->id);
                            $isolationArray  = $isolationdetails->toArray();


                            $isolationArray['name'] = $user->name;
                            $isolationArray['email_id'] =  $email_id;
                            $isolationArray['mail_subject'] = $mailsubject;

                            Mail::to($isolationArray['email_id'])->queue(new SWPIsolationEmail($isolationArray));
                        }
                    }
                }
                ...
                if (count($userids) > 0) {
                    ...
                    $notificationData = array(
                        ...
                        'mobile_notification' => json_encode(array(
                            'title' => $mailsubject,
                            'message' => 'New Isolation PTW - ' . $isolation->sub_permit_id . ' Requested by ' . getUsername($isolation->created_by),
                            'icon' => 'public/assets/images/icons/permit_to_work.png',
                            'id' => $isolation->id,
                            'module' => 2,
                        )),
                        'web_link' =>  admin_url('ptw/isolation/approvereject/' . encryptId($isolation->id)),
                ...
                Session::flash('success', 'Isolation PTW added successfully!');
            ...
            return redirect(admin_url('ptw/isolation/list'));
        } catch (Exception $ex) {
            report($ex);
            Session::flash('error', 'Something went wrong, Please try after sometimes!');
            return redirect(admin_url('ptw/isolation/list'));
    ...
    public function View(Request $request)
    {
        try {
            ...
            $isolation = $this->isolation->find($id);

            $locationDetails = $this->location->get();
            $ppelist = $this->category->getWhere(PTW_CAT_ISOLATION_PPE);

            $isolationpermitStatusLog = $this->permitstatuslog->getDetails(PTW_SUB_PERMIT_ISOLATION, $isolation->ptw_id, $id);
            ...
            return view('ptw.isolation.view', $data);
        } catch (Exception $ex) {
            report($ex);
        }
    }
    ...
    public function ApproveRejectSubmit(Request $request)
    {
        ...
        try {
            ...
            $this->isolation->statuschange($id, $ptw_status);

            $this->ptwsubpermit->statuschange($ptw_id, PTW_SUB_PERMIT_ISOLATION, $ptw_status);
            ...
            if ($is_reject == 1) {
                ...
                if (count($users) > 0) {
                    ...
                    foreach ($users as $user) {
                        ...
                        $mailsubject = "[PTW Notification - " . PTWID($ptw_id) . " ] " .  'Isolation PTW Request is Rejected';


                        if ($email_id != '' || $email_id != null) {

                            $isolationdetails =  $this->isolation->selectOne($permit->id);
                            $isolationArray  = $isolationdetails->toArray();
                ...
                if (count($userids) > 0) {
                    ...
                    $notificationData = array(
                        ...
                        'mobile_notification' => json_encode(array(
                            'title' => $mailsubject,
                            'message' => 'Isoaltion PTW - ' . $permit->sub_permit_id . ' is Rejected, Please update and resubmit.',
                            'icon' => 'public/assets/images/icons/permit_to_work.png',
                            'id' => $permit->id,
                            'module' => 2,
                        )),
                    ...
                    $notifydata = [
                        'title' => $mailsubject,
                        'message' => 'Isolation PTW - ' . $permit->sub_permit_id . ' is Rejected, Please update and resubmit.',
                    ];
                    mobilePushNotification($userId, $notifydata);
                }
            } else {
                ...
                if ($Assignedusers != null) {
                    ...
                    foreach ($Assignedusers as $user) {
                        ...
                        $mailsubject = "[PTW Notification - " . PTWID($ptw_id) . " ] " . 'Isolation PTW Request is Approved';
                ...
                if (count($userids) > 0) {
                    ...
                    $notificationData = array(
                        ...
                        'mobile_notification' => json_encode(array(
                            'title' => $mailsubject,
                            'message' => 'Isoaltion PTW - ' . $permit->sub_permit_id . ' is Approved.',
                            'icon' => 'public/assets/images/icons/permit_to_work.png',
                            'id' => $permit->id,
                            'module' => 2,
                        )),
                    ...
                    $notifydata = [
                        'title' => $mailsubject,
                        'message' => 'Isolation PTW - ' . $permit->sub_permit_id . ' is Approved',
                    ];
            ...
            $insert_array = array(
                'permit_type' => PTW_SUB_PERMIT_ISOLATION,
                'ptw_id' => $ptw_id,
                'sub_permit_id' => $id,
                'from_status' => 0,
    ...
    public function Edit(Request $request)
    {
        try {
            ...
            return view('ptw.isolation.edit', $data);
    ...
    public function Update(Request $request)
    {
        try {
            ...
            try {
                ...
                $ptwid  = $isolation->ptw_id;
                $general = $this->general->find($ptwid);

                $this->isolation->updates($general);

                $this->ptwsubpermit->updatePermit($ptwid, PTW_SUB_PERMIT_ISOLATION);

                $this->ptwsubpermit->statuschange($ptwid, PTW_SUB_PERMIT_ISOLATION, SUBPERMIT_STATUS_PENDING);
                ...
                if (count($users) > 0) {
                    ...
                    foreach ($users as $user) {
                        ...
                        $mailsubject = "[PTW Notification - " . PTWID($ptwid) . " ] " . 'Isolation PTW Resubmit for Approval';
                ...
                if (count($userids) > 0) {
                    ...
                    $notificationData = array(
                        ...
                        'mobile_notification' => json_encode(array(
                            'title' => $mailsubject,
                            'message' => 'Isolation PTW - ' . $isolation->sub_permit_id . ' Requested by ' . getUsername($isolation->created_by),
                            'icon' => 'public/assets/images/icons/permit_to_work.png',
                            'id' => $isolation->id,
                            'module' => 2,
                        )),
    ...
    public function ExportExcel(Request $request)
    {
        ...
        try {
            ...
            $allData = $this->isolation->exportdata();
            ...
            $writer = SimpleExcelWriter::streamDownload('Isolation Permit.xlsx')
                ->addHeader($header)
                ->addRows(
                    $exportData
                );
    ...
    public function ExportPdf(Request $request)
    {
        ...
        try {
            ...
            $view = view('ptw.isolation.pdf', $data);
            $html = $view->render();

            $mpdf->WriteHTML($html);

            $filename = "Isolation PTW.pdf";
    ...
    public function ExportViewPdf(Request $request)
    {
        ...
        try {
            ...
            $data = array(
                ...
                'pagetitle' => $isolation->sub_permit_id,
            ...
            $view = view('ptw.isolation.pdf.general', $data);
            $html = $view->render();
