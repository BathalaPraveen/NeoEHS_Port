...
use App\Mail\PTW\SWPDivingEmail;

class DivingPTWController extends Controller
{
    ...
    public function index(Request $request)
    {
                ...
                try {
                    ...
                    $datatables = Datatables::of($data['data'])
                        ...
                        ->addColumn('action', function ($row) {
                            $btn = '';
                            $btn = '<a href="' . admin_url('ptw/diving/view/' . encryptId($row->id)) . '"   class="" title="View"><i class="fa-solid fa-eye"></i></a> ';
                            if ($row->ptw_status == SUBPERMIT_STATUS_PENDING  && (CheckUserRole(ROLE_DIVING_PERMIT_APPROVER) || isAdmin() || CheckUserRole(ROLE_HSEUSER))) {
                                $btn .= '<a href="' . admin_url('ptw/diving/approvereject/' . encryptId($row->id)) . '"   class="" title="Approve / Reject"><i class="fa-solid fa-check-to-slot"></i></a> ';
                            }
                            if ($row->ptw_status == SUBPERMIT_STATUS_REJECTED && (Auth::id() == $row->created_by || isAdmin())) {
                                $btn .= '<a href="' . admin_url('ptw/diving/edit/' . encryptId($row->id)) . '"   class="" title="Edit"><i class="fa-solid fa-pen-to-square"></i></a> ';
                            }
                            $btn .= '<a href="' . admin_url('ptw/diving/view/pdf/' . encryptId($row->id)) . '" class=" " title="Pdf"><img style="width:20px;vertical-align: sub;" src="' . url('public/assets/images/icons/pdf.png') . '" alt="PDF" ></i> </a>';
                            return $btn;
                        })
        ...
        return view('ptw.diving.list', $data);
    }
    ...
    public function Add(Request $request)
    {
        ...
        try {
            ...
            $nextpermit = $this->ptwsubpermit->nextpermit($ptwid, PTW_SUB_PERMIT_DIVING);
            ...
            return view('ptw.diving.add', $data);
        } catch (Exception $ex) {
    ...
    public function Store(Request $request)
    {
        try {
            ...
            try {
                ...
                $diving = $this->diving->store($general);

                $this->ptwsubpermit->updatePermit($ptwid, PTW_SUB_PERMIT_DIVING);

                $nextpermit = $this->ptwsubpermit->nextpermit($ptwid, PTW_SUB_PERMIT_DIVING);

                Session::flash('success', 'Diving PTW added successfully!');
                ...
                $user_role = ROLE_DIVING_PERMIT_APPROVER;
                $notifywhere = array(
                ...
                if (count($users) > 0) {
                    ...
                    foreach ($users as $user) {
                        ...
                        $mailsubject = "[PTW Notification - " . PTWID($ptwid) . " ] " .  'New Diving PTW Request';


                        if ($email_id != '' || $email_id != null) {

                            $divingdetails =  $this->diving->selectOne($diving->id);
                            $divingArray  = $divingdetails->toArray();


                            $divingArray['name'] = $user->name;
                            $divingArray['email_id'] =  $email_id;
                            $divingArray['mail_subject'] = $mailsubject;

                            Mail::to($divingArray['email_id'])->queue(new SWPDivingEmail($divingArray));
                        }
                    }
                }
                ...
                if (count($userids) > 0) {
                    ...
                    $notificationData = array(
                        ...
                        'mobile_notification' => json_encode(array(
                            'title' => $mailsubject,
                            'message' => 'New Diving PTW - ' . $diving->sub_permit_id . ' Requested by ' . getUsername($diving->created_by),
                            'icon' => 'public/assets/images/icons/permit_to_work.png',
                            'id' => $diving->id,
                            'module' => 2,
                        )),
                        'web_link' =>  admin_url('ptw/diving/approvereject/' . encryptId($diving->id)),
    ...
    public function View(Request $request)
    {
        try {
            ...
            $diving = $this->diving->find($id);

            $locationDetails = $this->location->get();

            $equipmentgear = $this->category->getWhere(PTW_CAT_DIVING_EQUIPMENT);
            $divingsitepreparation = $this->category->getWhere(PTW_CAT_DIVING_SITE_PREPARATION);
            $divingpermitStatusLog = $this->permitstatuslog->getDetails(PTW_SUB_PERMIT_DIVING, $diving->ptw_id, $id);
            ...
            return view('ptw.diving.view', $data);
        } catch (Exception $ex) {
            report($ex);
        }
    }
    ...
    public function ApproveRejectSubmit(Request $request)
    {
        ...
        try {
            ...
            $this->diving->statuschange($id, $ptw_status);
            $this->ptwsubpermit->statuschange($ptw_id, PTW_SUB_PERMIT_DIVING, $ptw_status);
            ...
            if ($is_reject == 1) {
                ...
                if (count($users) > 0) {
                    ...
                    foreach ($users as $user) {
                        ...
                        $mailsubject = "[PTW Notification - " . PTWID($ptw_id) . " ] " .  'Diving PTW Request is Rejected';


                        if ($email_id != '' || $email_id != null) {

                            $divingdetails =  $this->diving->selectOne($permit->id);
                            $divingArray  = $divingdetails->toArray();
                ...
                if (count($userids) > 0) {
                    ...
                    $notificationData = array(
                        ...
                        'mobile_notification' => json_encode(array(
                            'title' => $mailsubject,
                            'message' => 'Diving PTW - ' . $permit->sub_permit_id . ' is Rejected, Please update and resubmit.',
                            'icon' => 'public/assets/images/icons/permit_to_work.png',
                            'id' => $permit->id,
                            'module' => 2,
                        )),
                ...
                if ($Assignedusers != null) {
                    ...
                    foreach ($Assignedusers as $user) {
                        ...
                        $mailsubject = "[PTW Notification - " . PTWID($ptw_id) . " ] " . 'Diving PTW Request is Approved';
            ...
            $insert_array = array(
                'permit_type' => PTW_SUB_PERMIT_DIVING,
                'ptw_id' => $ptw_id,
                'sub_permit_id' => $id,
                'from_status' => 0,
    ...
    public function Update(Request $request)
    {
        try {
            ...
            try {
                ...
                $this->diving->updates($general);

                $this->ptwsubpermit->updatePermit($ptwid, PTW_SUB_PERMIT_DIVING);

                $this->ptwsubpermit->statuschange($ptwid, PTW_SUB_PERMIT_DIVING, SUBPERMIT_STATUS_PENDING);
                ...
                if (count($users) > 0) {
                    ...
                    foreach ($users as $user) {
                        ...
                        $mailsubject = "[PTW Notification - " . PTWID($ptwid) . " ] " . 'Diving Work permit Resubmit';
                ...
                if (count($userids) > 0) {
                    ...
                    $notificationData = array(
                        ...
                        'mobile_notification' => json_encode(array(
                            'title' => $mailsubject,
                            'message' => 'Diving PTW - ' . $diving->sub_permit_id . ' Requested by ' . getUsername($diving->created_by),
                            'icon' => 'public/assets/images/icons/permit_to_work.png',
                            'id' => $diving->id,
                            'module' => 2,
                        )),
                ...
                Session::flash('success', 'Diving PTW updated successfully!');
                return redirect(admin_url('ptw/diving/list'));
            } catch (Exception $ex) {
    ...
    public function ExportExcel(Request $request)
    {
        ...
        try {
            ...
            $allData = $this->diving->exportdata();
            ...
            $writer = SimpleExcelWriter::streamDownload('Diving Permit.xlsx')
                ->addHeader($header)
                ->addRows(
                    $exportData
                );
    ...
    public function ExportViewPdf(Request $request)
    {
        ...
        try {
            ...
            $data = array(
                ...
                'pagetitle' => $diving->sub_permit_id,
            ...
            $view = view('ptw.diving.pdf.general', $data);
            $html = $view->render();
