...
use App\Mail\PTW\SWPGasEmail;

class GasPTWController extends Controller
{
                ...
                try {
                    ...
                    $datatables = Datatables::of($data['data'])
                        ...
                        ->addColumn('action', function ($row) {
                            $btn = '';
                            $btn = '<a href="' . admin_url('ptw/gastest/view/' . encryptId($row->id)) . '"   class="" title="View"><i class="fa-solid fa-eye"></i></a> ';

                            if ($row->ptw_status == SUBPERMIT_STATUS_PENDING  && (CheckUserRole(ROLE_GAS_PERMIT_APPROVER) || isAdmin() || CheckUserRole(ROLE_HSEUSER))) {
                                $btn .= '<a href="' . admin_url('ptw/gastest/approvereject/' . encryptId($row->id)) . '"   class="" title="Approve / Reject"><i class="fa-solid fa-check-to-slot"></i></a> ';
                            }
                            if ($row->ptw_status == SUBPERMIT_STATUS_REJECTED && (Auth::id() == $row->created_by || isAdmin())) {

                                $btn .= '<a href="' . admin_url('ptw/gastest/edit/' . encryptId($row->id)) . '"   class="" title="Edit"><i class="fa-solid fa-pen-to-square"></i></a> ';
                            }

                            $btn .= '<a href="' . admin_url('ptw/gastest/view/pdf/' . encryptId($row->id)) . '" class=" " title="Pdf"><img style="width:20px;vertical-align: sub;" src="' . url('public/assets/images/icons/pdf.png') . '" alt="PDF" ></i> </a>';
                            return $btn;
                        })
    ...
    public function Add(Request $request)
    {
        ...
        try {
            ...
            $nextpermit = $this->ptwsubpermit->nextpermit($ptwid, PTW_SUB_PERMIT_GAS);
    ...
    public function Store(Request $request)
    {
        try {
            ...
            $messages = [
                'ptwid.required' => 'PTW id is missing',
                'workdescription.required' => 'Please enter Work description',
                'accept_terms.required' => 'PTW not Accepted',
            ];
            ...
            try {
                ...
                $gas =  $this->gas->store($general);

                $this->ptwsubpermit->updatePermit($ptwid, PTW_SUB_PERMIT_GAS);
                ...
                $user_role = ROLE_GAS_PERMIT_APPROVER;
                $notifywhere = array(
                ...
                if (count($users) > 0) {
                    ...
                    foreach ($users as $user) {
                        ...
                        $mailsubject = "[PTW Notification - " . PTWID($ptwid) . " ] " .'New Gas PTW Request';


                        if ($email_id != '' || $email_id != null) {

                            $gasdetails =  $this->gas->selectOne($gas->id);
                            $gasArray  = $gasdetails->toArray();


                            $gasArray['name'] = $user->name;
                            $gasArray['email_id'] =  $email_id;
                            $gasArray['mail_subject'] = $mailsubject;

                            Mail::to($gasArray['email_id'])->queue(new SWPGasEmail($gasArray));
                        }
                    }
                }
                ...
                if (count($userids) > 0) {
                    ...
                    $notificationData = array(
                        ...
                        'mobile_notification' => json_encode(array(
                            'title' => $mailsubject,
                            'message' => 'New Gas PTW - ' . $gas->sub_permit_id . ' Requested by ' . getUsername($gas->created_by),
                            'icon' => 'public/assets/images/icons/permit_to_work.png',
                            'id' => $gas->id,
                            'module' => 2,
                        )),
                        'web_link' =>  admin_url('ptw/gastest/approvereject/' . encryptId($gas->id)),
                ...
                Session::flash('success', 'Gas PTW added successfully!');
            ...
            return redirect(admin_url('ptw/gastest/list'));
        } catch (Exception $ex) {
            report($ex);
            Session::flash('error', 'Something went wrong, Please try after sometimes!');
            return redirect(admin_url('ptw/gastest/list'));
    ...
    public function View(Request $request)
    {
        try {
            ...
            $gaspermit = $this->gas->find($id);

            $gaspermitStatusLog = $this->permitstatuslog->getDetails(PTW_SUB_PERMIT_GAS, $gaspermit->ptw_id, $id);
    ...
    public function ApproveRejectSubmit(Request $request)
    {
        ...
        try {
            ...
            $this->gas->statuschange($id, $ptw_status);

            $this->ptwsubpermit->statuschange($ptw_id, PTW_SUB_PERMIT_GAS, $ptw_status);
            ...
            if ($is_reject == 1) {
                ...
                if (count($users) > 0) {
                    ...
                    foreach ($users as $user) {
                        ...
                        $mailsubject = "[PTW Notification - " . PTWID($ptw_id) . " ] " .'Gas PTW Request is Rejected';


                        if ($email_id != '' || $email_id != null) {

                            $gasdetails =  $this->gas->selectOne($permit->id);
                            $gasArray  = $gasdetails->toArray();
                ...
                if (count($userids) > 0) {
                    ...
                    $notificationData = array(
                        ...
                        'mobile_notification' => json_encode(array(
                            'title' => $mailsubject,
                            'message' => 'Gas PTW - ' . $permit->sub_permit_id . ' is Rejected, Please update and resubmit.',
                            'icon' => 'public/assets/images/icons/permit_to_work.png',
                            'id' => $permit->id,
                            'module' => 2,
                        )),
                        'web_link' =>  admin_url('ptw/gastest/edit/' . encryptId($permit->id)),
                        'assigned_user' => array_to_string($userids),
                        'created_by' => Auth::id(),
                    );
                    notificationSave($notificationData);
            ...
            $insert_array = array(
                'permit_type' => PTW_SUB_PERMIT_GAS,
                'ptw_id' => $ptw_id,
                'sub_permit_id' => $id,
                'from_status' => 0,
    ...
    public function Update(Request $request)
    {
        try {
            ...
            try {
                ...
                $this->gas->updates($general);

                $this->ptwsubpermit->updatePermit($ptwid, PTW_SUB_PERMIT_GAS);


                $this->ptwsubpermit->statuschange($ptwid, PTW_SUB_PERMIT_GAS, SUBPERMIT_STATUS_PENDING);
                 ...
                 */
                 ...
                 if (count($users) > 0) {
                     ...
                     foreach ($users as $user) {
                         ...
                         $mailsubject = "[PTW Notification - " . PTWID($ptwid) . " ] " .'Gas Test PTW Resubmited for Approval';
                 ...
                 if (count($userids) > 0) {
                     ...
                     $notificationData = array(
                         ...
                         'mobile_notification' => json_encode(array(
                             'title' => $mailsubject,
                             'message' => 'Gas PTW - ' . $gas->sub_permit_id . ' Requested by ' . getUsername($gas->created_by),
                             'icon' => 'public/assets/images/icons/permit_to_work.png',
                             'id' => $gas->id,
                             'module' => 2,
                         )),
                ...
                Session::flash('success', 'Gas PTW Updated successfully!');
                return redirect(admin_url('ptw/gastest/list'));
            } catch (Exception $ex) {
    ...
    public function ExportExcel(Request $request)
    {
        ...
        try {
            ...
            $writer = SimpleExcelWriter::streamDownload('Gas Permit.xlsx')
                ->addHeader($header)
                ->addRows(
                    $exportData
                );
    ...
    public function ExportViewPdf(Request $request)
    {
        ...
        try {
            ...
            // $view = view('ptw.gas.pdf.general', $data);
