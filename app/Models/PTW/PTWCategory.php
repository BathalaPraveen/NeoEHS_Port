<?php

namespace App\Models\PTW;

use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Support\Facades\Auth;


use App\Scopes\TrashScope;

class PTWCategory extends Model
{
    use  HasFactory;


    protected $table = 'ptw_master_category';
    protected $primaryKey = 'id';

    protected $fillable = [
        'activity_id',
        'category_name',
        'input_type',
        'required',
        'other_params',
        'status',
        'trash',
        'created_by',
        'updated_by',
        'created_at',
        'updated_at'

    ];

    protected $attributes = [
        'status' => 1,
        'trash' => 'NO',
    ];

    public function list()
    {
        $request = request();
        $search = '';

        $query = $this->select('ptw_master_category.*','ptw_master_activity.activity_name','ptw_master_activity.module');
        $query = $query->leftJoin('ptw_master_activity','ptw_master_category.activity_id' , '=' ,'ptw_master_activity.id');


       
        $data_count = $query->count();
        $total_records = $data_count;

        $query->orderBy('id','DESC');


        if ($request->length != -1) {
            $query->offset($request->start)->limit($request->length);
        }

        $data = $query->get();

        $datas = [
            'data' => $data,
            'total_records' => $total_records
        ];

        return $datas;
    }

    public function store()
    {

        $request = request();

        $insert_array = array(
            'activity_id' => decryptId($request->activity),
            'category_name' => $request->category_name,
            'created_by' => Auth::id()
        );

        return $this->create($insert_array);
    }

    public function updates($id)
    {

        $request = request();

        $update_array = array(
            'activity_id' => decryptId($request->activity),
            'category_name' => $request->category_name,
            'updated_by' => Auth::id()
        );

        return $this->where('id', $id)->update($update_array);
    }

    public function statuschange($id)
    {
        $request = request();

        $type = $request->types;
        if ($type == 1) {
            $update_data = array(
                'status' => 0,
            );
        } else {
            $update_data = array(
                'status' => 1,
            );
        }

        return $this->where('id', $id)->update($update_data);
    }

    public function deleterecord($id)
    {

        $update_data = array(
            'status' => 0,
            'trash' => 'YES',
        );

        return $this->where('id', $id)->update($update_data);
    }

    public function exportdata()
    {
        $request = request();
        $search = '';

        $query = $this->select('ptw_master_category.*','ptw_master_activity.activity_name','ptw_master_activity.module');
        $query = $query->leftJoin('ptw_master_activity','ptw_master_category.activity_id' , '=' ,'ptw_master_activity.id');


       

        $query = $query->orderBy('id', 'Desc');
        return  $query->get();
    }

        public function selectOne($id)
    {

        $data = $this->select('*')
            ->where('id', $id)
            ->first();

        return $data;
    }

    public function selectOneWhere($where)
    {

        $data = $this->select('*')
            ->where($where)
            ->first();

        return $data;
    }

    public function ajaxList($activity = ''){

        $query = $this->select('id','category_name');

        if($activity != ''){
            $query = $query->where('activity_id',$activity);
        }

        $datas = $query->get();

        $list = [];
        foreach($datas as $data){
            $listvalue = [];
            $listvalue['id'] = encryptId($data->id);
            $listvalue['name'] = $data->category_name;

            $list[] = $listvalue;
        }
        return $list;

    }

    public function getWhere($activity = ''){

        $query = $this->select('id','category_name','input_type','required','other_params');

        if($activity != ''){
            $query = $query->where('activity_id',$activity);
        }

        $datas = $query->orderby('sort_order', 'ASC')->get();


        return $datas;

    }


    protected static function booted()
    {
        static::addGlobalScope(new TrashScope('ptw_master_category'));


    }
}
