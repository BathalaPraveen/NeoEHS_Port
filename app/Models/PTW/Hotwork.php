...
class Hotwork extends Model
{
    ...
    protected $table = 'ptw_sub_permit_hotwork';
    ...
    public function list()
    {
        ...
        $query = $this->select('ptw_sub_permit_hotwork.*', 'master_location.location_name', 'users.name');
        $query = $query->leftJoin('master_location', 'ptw_sub_permit_hotwork.location', '=', 'master_location.id');
        $query = $query->leftJoin('users', 'ptw_sub_permit_hotwork.created_by', '=', 'users.id');

        if ($request->has('status') && $request->status != '') {
            $status = decryptId($request->status);
            $query->Where('ptw_sub_permit_hotwork.ptw_status', $status);
        }

        if ($request->has('location') && $request->location != '') {
            $location = decryptId($request->location);
            $query->Where('ptw_sub_permit_hotwork.location', $location);
        }
        ...
        if (!in_array(ROLE_SUPERADMIN, getUserRoleId(Auth::id())) && !in_array(ROLE_ADMIN, getUserRoleId(Auth::id()))) {
            ...
            if (in_array(ROLE_CONTRACTORUSER, getUserRoleId(Auth::id())) || in_array(ROLE_CONTRACTORADMIN, getUserRoleId(Auth::id())) || in_array(ROLE_NORMAL_USER, getUserRoleId(Auth::id())) ) {
                $query = $query->where('ptw_sub_permit_hotwork.created_by', Auth::id());
            }else if(! in_array(ROLE_HOTWORK_PERMIT_APPROVER, getUserRoleId(Auth::id()))){
    ...
    public function store($general)
    {
        ...
        $insert_array = array(
            ...
            'locationmarine' => $request->locationmarine,
            ...
            'hotworkoperation' => json_encode(arrayDecrypt($request->hotworkoperation)),
            'precautions' => json_encode($request->precautions),
    ...
    {
        static::addGlobalScope(new TrashScope('ptw_sub_permit_hotwork'));

        static::created(function ($model) {

            $uniqueId = 'PTW-HOT-' . str_pad($model->id, 5, '0', STR_PAD_LEFT);
