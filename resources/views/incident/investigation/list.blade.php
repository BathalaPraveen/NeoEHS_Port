...
@section('title', 'Incident Investigation List')
                ...
                <div class="card-body">
                    <div class="d-lg-flex align-items-center gap-3">
                        ...
                        <div class="position-relative">
                            <h5 class="card-title">Incident Investigation List</h5>
                        </div>
                        <div class="ms-auto">
                            ...
                            {{-- <a href="{{ admin_url('incident/investigation/add') }}" class="btn btn-primary">
                                New
                            </a> --}}
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            var table = $('.datatable-list').DataTable({
                ...
                "ordering": false,
                ajax: {
                    url: "{{ admin_url('incident/investigation/list') }}",
                    type: 'POST',
                    headers: {
            ...
            $(document).on('click', '.StatusChange', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ admin_url('incident/investigation/status') }}",
                            type: 'post',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]')
                                    .attr('content')
            ...
            $(document).on('click', '.recordDelete', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ admin_url('incident/investigation/delete') }}",
                            type: 'post',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]')
                                    .attr('content')
