...
@section('title', 'Incident Category List')
@section('pageurl', admin_url('incident/master/category/list'))
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            $(document).on('click', '.StatusChange', function() {
                ...
                if (types == 1) {
                    var title = 'Do you want to In-Activate the Category';
                    var text = 'In-Activate';
                    var btncolor = '#dc3545'

                } else {
                    var title = 'Do you want to Activate the Category';
                    var text = 'Activate';
                    var btncolor = '#7ddc35'
                }
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ admin_url('incident/master/category/status') }}",
                            type: 'post',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]')
                                    .attr('content')
            ...
            $(document).on('click', '.recordDelete', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ admin_url('incident/master/category/delete') }}",
                            type: 'post',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]')
                                    .attr('content')
