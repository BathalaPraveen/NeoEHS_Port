...
@section('title', 'Specific Location Edit')
                ...
                <div class="">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Specific Location Edit</li>
                        </ol>
                    </nav>
                </div>
                ...
                <div class="col">
                    ...
                    <div class="card border-top border-0 border-4 border-primary">
                        <div class="card-body">
                            ...
                            <hr />
                            <form class="" id="specific_location_edit" novalidate method="POST"
                                action="{{ admin_url('specificlocation/edit/submit') }}">
                                <div class="p-4 border rounded">
                                    <div class="row">
                                        <input type="hidden" name="id" value="{{ encryptId($location->id) }}">
                                        @csrf
                                        ...
                                        <div class="col-md-3 form-input">
                                            ...
                                            <input type="text" name="specific_location_id" class="form-control"
                                                id="specific_location_id" value="{{ $location->specific_location_id }}"
                                                readonly required>
                                        </div>
                                        <div class="col-md-3 form-input">
                                            ...
                                            <select name="company_id" id="company_id" class="form-control select2">
                                                ...
                                                @foreach ($companyDetails as $company)
                                                    <option value="{{ encryptId($company->id) }}"
                                                        @selected($company->id == $location->company_id)>
                                            ...
                                            <select name="location_id" id="location_id" class="form-control select2"
                                                ...
                                                @foreach ($locationDetails as $loc)
                                                    <option value="{{ encryptId($loc->id) }}"
                                                        @if ($loc->id == $location->location_id) selected @endif>
                                        ...
                                        <div class="col-md-3 form-input">
                                            ...
                                            <input type="text" name="specific_location_name" class="form-control"
                                                value="{{ $location->specific_loc_name }}" id="specific_location_name"
                                                value="" required>
                                        </div>
                                            ...
                                            <select name="area_owner[]" id="area_owner" class="form-control select2"
                                                ...
                                                @foreach ($employeeDetails as $employee)
                                                    <option value="{{ encryptId($employee->id) }}"
                                                        {{ in_array($employee->id, explode(',', $location->area_owner)) ? 'selected' : '' }}>
                                                        {{ $employee->name }}
                                                    </option>
                                                @endforeach
                                        ...
                                        <div class="col-md-12 form-input">
                                            <label for="specific_location_desc" class="form-label require">Specific Location
                                                Address / Description</label>
                                            <textarea class="form-control" required name="specific_location_desc" id="specific_location_desc" rows="5">{{ $location->specific_loc_desc }}</textarea>
                                        </div>
...
@push('script')
    <script type="text/javascript">
        ...
        $(function() {
            $('#specific_location_edit').validate({
                rules: {
