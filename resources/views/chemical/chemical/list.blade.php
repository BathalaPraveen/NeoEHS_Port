                ...
                <div class="card-body">
                    <div class="d-lg-flex align-items-center gap-3">
                        ...
                        <div class="position-relative">
                            <h5 class="card-title">Chemical List</h5>
                        </div>
                        <div class="ms-auto">
                            ...
                            <a href="{{ admin_url('chemical/chemical/add') }}" class="btn btn-primary">
                                New
                            </a>
                        </div>
                    </div>
                                ...
                                <div class="col-md-12">
                                    <div class="row">
                                        ...
                                        <div class="col-md-3 form-input">
                                            <label for="chemicaltype" class="form-label ">Chemical Type</label>
                                            <select name="chemicaltype" id="chemicaltype" class="form-control select2"
                                                style="width: 100%">
                                                <option value="">Select Chemical Type </option>
                                                @foreach($chemicalmaster as $master)
                                                <option value="{{encryptId($master ->id)}}">
                                                    {{$master ->chemical_name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-md-3 form-input">
                                            ...
                                            <select name="status" id="status" style="width: 100%"
                                                ...
                                                @foreach($chemicalstatus as $status)
                                                    <option value="{{encryptId($status ->id)}}">
                                                        {{$status ->chemical_status}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-3">
    ...
    <script type="text/javascript">
        $(function() {
            ...
            var table = $('.datatable-list').DataTable({
                ...
                "ordering": false,
                ajax: {
                    url: "{{ admin_url('chemical/chemical/list') }}",
                    type: 'POST',
                    headers: {
                    ...
                    data: function(d) {
                        d.chemicaltype = $("#chemicaltype").val();
                        d.status = $("#status").val();
                ...
                buttons: [{
                        ...
                        buttons: [{
                                ...
                                action: function(e, dt, button, config) {
                                    ...
                                    var chemicaltype = $("#chemicaltype").val();
                                    var status = $("#status").val();


                                    window.location.href =
                                        "{{ admin_url('chemical/chemical/export/pdf') }}" +
                                        '?search=' + searchValue +
                            ...
                            },
                            {
                                ...
                                action: function(e, dt, button, config) {
                                    ...
                                    window.location.href =
                                        "{{ admin_url('chemical/chemical/export/excel') }}" +
                                        '?search=' + searchValue +
                                        '&chemicaltype=' + chemicaltype +
                                        '&status=' + status;
                                }
            ...
            $(document).on('click', '.StatusChange', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ admin_url('chemical/chemical/status') }}",
                            type: 'post',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]')
                                    .attr('content')
            ...
            $(document).on('click', '.recordDelete', function() {
                ...
                var title = 'Do you want to delete the Chemical';
                var text = 'Delete';
                var btncolor = '#dc3545'
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ admin_url('chemical/chemical/delete') }}",
                            type: 'post',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]')
                                    .attr('content')
