<div>
    <form action="{{ admin_url('ptw/master/category/add/submit') }}" id="category_add" method="POST" novalidate>
        @csrf
        <div class="modal-header">
            <h5 class="modal-title">PTW Category Add</h5>
                ...
                <div class="col-sm-9 form-input">
                    <select name="activity" class="select2 form-control" style="width: 100%">
                        ...
                        @foreach ($activityList as $activity )
                            <option value="{{ encryptId($activity->id)}}">{{$activity->activity_name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
